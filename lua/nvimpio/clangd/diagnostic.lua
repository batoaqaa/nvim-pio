...
local function inject_into_clangd(code, message)
  ...
  -- FIX 1: Enforce explicit double-quoted signature comparisons to prevent double appends
  local search_signature = '"' .. target_item .. '"'
  if string.find(file_content, search_signature, 1, true) then
    return false -- Already handled, exit early gracefully
  end

  -- FIX 2: Correctly reference bracket positions from the table split array
  ...
  -- Fallback logic if the YAML template formatting block was manually edited/altered
