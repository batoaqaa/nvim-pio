...
function M.manage_file_diagnostics_interactive(state_override)
  ...
  vim.ui.select(items, {
      ...
      -- ========================================================================
      -- 🛑 FINISHED GATE: User pressed Escape or 'q' to close out the menu sequence
      -- ========================================================================
      if not choice then
        -- ONE-TIME FINAL DISK WRITE POINT
        local f = io.open(filter_db_path, 'wb')
        if f then
          local payload = { codes = active_file_blocked, flags = M.removed_flags }
          f:write(require('nvimpio.utils.misc').jsonFormat(payload))
        ...
        -- Flush session cache arrays safely entirely out of RAM memory on final exit
        for k in pairs(M.session_discovered_codes) do M.session_discovered_codes[k] = nil end

        -- Refresh buffer lints viewport tracking maps cleanly
        vim.schedule(function()
          if vim.api.nvim_buf_is_valid(bufnr) then
            vim.api.nvim_buf_call(bufnr, function()
              local old = vim.o.shortmess
        ...
        return -- Exit loop sequence cleanly
      end

      -- ========================================================================
      -- 🔄 ACTIVE INTERACTION GATE: User clicked an item to toggle or view
      -- ========================================================================
      if choice.action == 'none' then
        -- Keep menu open on read-only rows without state alterations
        M.manage_file_diagnostics_interactive(active_file_blocked)
        return
      end

      -- Mutate the active_file_blocked table entries inside temporary runtime memory
      if choice.action == 'reset' then
        active_file_blocked = {}
      elseif choice.action == 'block' then
        active_file_blocked[choice.id] = true
      ...
      -- Loop right back into the selection framework with our in-memory changes preserved
      M.manage_file_diagnostics_interactive(active_file_blocked)
    end)
    --------------------------------------------------------------------------------------------
  end
