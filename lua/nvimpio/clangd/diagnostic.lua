...
-- 🌟 FIXED: Force permanent instantiation as empty tables right at the top
-- This completely prevents 'nil' indexing crashes under rapid startup race conditions
...
local function load_filter_database(bufnr)
  -- Always fall back to safe empty tracking allocations if disk reads fail
  M.manual_blocked_codes = {}
  M.removed_flags = {}
...
-- Initialize the baseline database state variables safely on loading
...
function M.diagnostic_handler(err, result, ctx, config)
  ...
  for _, diag in ipairs(result.diagnostics) do
    ...
    if code and type(code) == 'string' and code ~= '' then
      -- Automatically target unknown compiler arguments instantly
      if code == 'drv_unknown_argument' or code == 'drv_unknown_argument_with_suggestion' or code == 'fatal_too_many_errors' then
        -- Simple, robust capture that grabs any word starting with a dash (-)
