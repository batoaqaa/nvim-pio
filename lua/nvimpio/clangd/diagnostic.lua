...
local function load_filter_database()
  ...
  if not M.blocked_codes then M.blocked_codes = {} end
  if not M.removed_flags then M.removed_flags = {} end
end
...
local function save_filter_database()
  ...
  boiler.remove = {}
  for flag, _ in pairs(M.removed_flags or {}) do table.insert(boiler.remove, flag) end
  table.sort(boiler.remove)

  boiler.suppress = {}
  for code, _ in pairs(M.blocked_codes or {}) do table.insert(boiler.suppress, code) end
...
function M.manage_file_diagnostics_interactive()
  ...
  local function open_dashboard_loop()
    ...
    if #raw_diagnostics == 0 then raw_diagnostics = vim.diagnostic.get(current_buf) end
    ...
    for _, diag in ipairs(raw_diagnostics) do
      ...
      if type(code_name) == 'string' and code_name:find('drv_unknown') then is_driver_error = true end
    ...
    table.sort(unblock_options, function(a, b) return a.display < b.display end)
    for _, opt in ipairs(unblock_options) do table.insert(dashboard_items, opt) end
    ...
    vim.ui.select(dashboard_items, {
      ...
      elseif choice.action == 'block_code' then M.blocked_codes[choice.id] = true
      elseif choice.action == 'unblock_flag' then M.removed_flags[choice.id] = nil
      elseif choice.action == 'unblock_code' then M.blocked_codes[choice.id] = nil end

      save_filter_database()

      vim.schedule(function() open_dashboard_loop() end)
    end)
  end
  open_dashboard_loop()
end
