...
-- "--fallback-style=llvm" --compile_args_from=filesystem -- -Wno-unknown-argument && echo _CMMNDS_0001:DONE || echo _CMMNDS_0001:FAIL
    local cmd_str = string.format('%s --compile-commands-dir=. --check=%s --query-driver=%s --log=error --fallback-style=llvm --compile_args_from=filesystem', clangdCmd, check_file, _G.metadata.query_driver)
    local pio = require('nvimpio.pio.upkeep')
