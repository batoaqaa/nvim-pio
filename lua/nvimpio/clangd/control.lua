...
function M.manage_file_diagnostics_interactive()
  ...
  -- Target file location: Root path of your current open project workspace (.clangd)
  ...
  vim.ui.select(unique_items, {
    prompt = " 🛠️ Select an LSP Error to Suppress Permanently inside Local .clangd ",
    ...
    -- A. INITIALIZE FILE: If .clangd is missing, build your exact required static blueprint layout
...
CompileFlags:
  Remove: [
    ...
    -- B. READ THE EXISTING CONTENT FROM DISK BINARY STREAM
    local file_content = ""
    ...
    -- C. STRATEGY ASSIGNMENT: Parse Driver Execution Flag vs Standard Code Warning
    local target_item = ""
    ...
    if is_driver_flag then
      -- Extract raw parameter payload out of the error sentence (e.g. gets "-mlongcalls")
      ...
      -- Standard warning codes go in completely raw without quotes
    ...
    if string.find(file_content, target_item, 1, true) then
      vim.notify("ℹ️ '" .. target_item .. "' is already mapped inside your .clangd file.", vim.log.levels.INFO)
      return
    end

    -- D. 🌟 SURGICAL INJECTION: Find the marker line and append directly into its block array
    local updated_content = ""
    if is_driver_flag then
      -- Inject into the Dynamic Block's CompileFlags.Remove section
      local pattern = "(# Dynamic configuration block generated by nvim%-pio%s*\n%s*CompileFlags:%s*\n%s*Remove:%s*%[)"
      local replacement = "# Dynamic configuration block generated by nvim-pio\nCompileFlags:\n  Remove: [\n    \"" .. target_item .. "\","
      updated_content = string.gsub(file_content, pattern, replacement)
    else
      -- Inject into the Dynamic Block by appending a dedicated custom Diagnostic Suppress tree at the bottom
      -- This matches the YAML standard format layout for document anchors cleanly
      updated_content = file_content .. "\nDiagnostics:\n  Suppress: [\n    " .. target_item .. ",\n  ]"
    end

    -- E. COMMIT DATA AND TRIGGER BACKGROUND SERVER REBOOT
