...
local function IsPioTerminal(win)
  if not win or not vim.api.nvim_win_is_valid(win) then
    return false
  end
  local buf = vim.api.nvim_win_get_buf(win)
  return buf == pio_cli_buf or buf == pio_mon_buf
end

local function FindSafeWorkspaceWindow()
  if last_active_editor_win and vim.api.nvim_win_is_valid(last_active_editor_win) then
    if not IsPioTerminal(last_active_editor_win) then
      return last_active_editor_win
    end
  end
  for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
    if vim.api.nvim_win_is_valid(win) and not IsPioTerminal(win) then
      local buf = vim.api.nvim_win_get_buf(win)
...
local function HideTerminalWindow(terminal_type)
  ...
  if win_id and vim.api.nvim_win_is_valid(win_id) then
    local safe_target_win = FindSafeWorkspaceWindow()
    if safe_target_win and safe_target_win ~= win_id then
...
function M.ToggleTerminal(command, terminal_type)
  ...
  if not IsPioTerminal(active_win) then
  ...
  local safe_editor_win = FindSafeWorkspaceWindow()
...
vim.api.nvim_create_autocmd({ 'WinNew', 'WinClosed', 'TabEnter', 'BufWinEnter' }, {
  ...
  callback = function(args)
    if args.buf == pio_cli_buf or args.buf == pio_mon_buf then
      return
    end
    ...
    vim.schedule(function()
      ...
      if is_misaligned then
        ...
        if current_win and vim.api.nvim_win_is_valid(current_win) and current_win ~= pio_win then
          if not IsPioTerminal(current_win) then
            vim.api.nvim_set_current_win(current_win)
          else
            local fallback = FindSafeWorkspaceWindow()
            if fallback then
