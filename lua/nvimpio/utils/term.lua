...
local function UpdateWinbarTitles()
  local cli_open = IsTerminalOpen("cli")
  local mon_open = IsTerminalOpen("monitor")

  -----------------------------------------------------------------------------
  -- 🔍 WINBAR TELEMETRY TRACE
  -----------------------------------------------------------------------------
  vim.notify(string.format("[WINBAR TRACE] cli_open: %s (buf:%s, win:%s) | mon_open: %s (buf:%s, win:%s)",
    tostring(cli_open), tostring(state.cli.buf), tostring(state.cli.win),
    tostring(mon_open), tostring(state.monitor.buf), tostring(state.monitor.win)), vim.log.levels.INFO)
  -----------------------------------------------------------------------------

  local hint = (cli_open and mon_open) and " [;; Switch] " or " [; Hide] "
...
function M.ToggleTerminal(command, terminal_type)
  ...
  if IsTerminalOpen(opposite_type) then
...
-- M.exit_callback = nil
--
-- -- The Absolute State Machine Matrix
-- local state = {
--   cli = { buf = nil, win = nil, title = ' Pio CLI> ' },
--   monitor = { buf = nil, win = nil, title = ' Pio Monitor ' },
-- }
...
-- local content = ''
--
-- -- Safe runtime check to see if a terminal partition is open on screen
-- local function IsTerminalOpen(term_type)
--   local s = state[term_type]
--   return s.win and vim.api.nvim_win_is_valid(s.win) and vim.api.nvim_win_get_buf(s.win) == s.buf
-- end
--
-- local function UpdateWinbarTitles()
--   local hint = (M.IsTerminalOpen('cli') and M.IsTerminalOpen('monitor')) and ' [;; Switch] ' or ' [; Hide] '
--   vim.api.nvim_set_hl(0, 'PioWinBar', { bg = M.config.winbar_bg, fg = M.config.winbar_fg })
--
--   for _, t in pairs({ 'cli', 'monitor' }) do
--     local s = state[t]
...
--   return IsTerminalOpen(term_type)
-- end
--
-- function M.ToggleTerminal(command, terminal_type)
