  ...
  local function inject_into_ini()
    ...
    if f_in then
      for line in f_in:lines() do
        -- STRIP FILTER: Remove any lines generated by previous runs of this wizard
        if
          not line:find('^upload_port%s*=')
          and not line:find('^monitor_port%s*=')
          and not line:find('^upload_speed%s*=')
          and not line:find('^monitor_speed%s*=')
          and not line:find('^monitor_filters%s*=')
        then
    ...
    -- Compile our new active parameters into standard PlatformIO syntax properties strings
    local new_configs = {}
    ...
    -- Dynamically inject hardware resets signals filters options strings
    if p_state.monitor_rts or p_state.monitor_dtr then
      local rts = p_state.monitor_rts or '1'
      local dtr = p_state.monitor_dtr or '1'
      table.insert(new_configs, string.format('monitor_filters = direct, send_on_enter, rts_toggle_%s, dtr_toggle_%s', rts, dtr))
    end

    -- Locate or inject a clean global [env] block right at the top of the configurations file array
    local env_found = false
    for idx, line in ipairs(lines) do
      if line:match('^%s*%[%s*env%s*%]%s*$') then
        env_found = true
        -- Inject choices immediately inside the existing global block context
    ...
    -- If no [env] block exists in their file layout yet, append a brand new one straight to the bottom
    ...
    -- Write the structural modifications array safely back down to your project hard drive
