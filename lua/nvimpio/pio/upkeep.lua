...
function M.configure_hardware_parameters()
  ...
  local function inject_into_ini()
    ...
    if f_in then
      ...
      -- FIX: Changed matching mechanism to keep empty lines intact!
      -- This matches the line contents and handles line endings safely.
      for line in src:gmatch("([^\r\n]*)\r?\n?") do
        -- Skip the trailing match block generated by gmatch at the EOF
        if line ~= "" or src:sub(-1) == "\n" then
          if
      ...
      -- Remove duplicate trailing element artifact caused by the gmatch line capture
    ...
    if env_idx then
      -- FIX: Iterate backwards when inserting items into arrays by index position.
      -- This guarantees configuration variables inject in the correct structural order!
