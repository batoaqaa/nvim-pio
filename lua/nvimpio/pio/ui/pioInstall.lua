...
local function pioInstall(runtime_dir, on_done)
  ...
  local install_cmd, pioUpgrade_cmd, pipUpgrade_cmd, pioEnv
  if OS.is_win then
  ...
  require('nvimpio.device.parser').run_sequence({ cmnds = { download_cmd, install_cmd, pipUpgrade_cmd, pioUpgrade_cmd, pioEnv }, cb = cb, from = 'PioInstall:' })
end
