...
local function render_helix_float(title, items, on_back)
  ...
  -- Append structural backtracking navigation hint if inside a nested submenu branch
  if on_back then
    table.insert(display_lines, string.rep('─', max_desc_len + 14))
    table.insert(display_lines, '  Back' .. string.rep(' ', max_desc_len - 2) .. '[<BS>] ')
  end

  -- 3. Open a temporary unlisted scratchpad canvas buffer
  ...
  -- Apply professional Neovim float palette background highlight configs
  ...
  for shortcut, item in pairs(key_mappings) do
    vim.keymap.set('n', shortcut, function()
      ...
      if item.node == 'item' then
        -- Native string command execution
...
---Processes the configuration parameters and initializes the native launch hook
function M.buildUserMenu(config)
  if config == nil or config.menu_key == nil then
    return
  end

  -- 1. Create a native Neovim keymap that triggers before Which-Key can process anything
  vim.keymap.set('n', config.menu_key, function()
    ...
    -- Immediately render your pristine, flawless native Helix panel overlay window
