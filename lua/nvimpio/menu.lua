...
function M.buildUserMenu(config)
  ...
  -- =========================================================================
  -- THE ULTIMATE INTERCEPT ACTION PROXY
  -- =========================================================================
  -- Register your menu key directly inside Which-Key using a function callback action instead of a subtree.
  -- This approach provides stable performance across any user setup.
  wk.add({
    {
      config.menu_key,
      desc = config.menu_name,
      icon = { icon = '  ', color = 'orange' },
      -- Using a function action allows your menu key to show up cleanly in top-level leader menus,
      -- but hitting it seamlessly drops straight out to our custom layout frame instead!
      function()
        -- Close Which-Key's open active dashboard lookup window buffer immediately
        pcall(function()
          require('which-key.view').close()
        end)

        -- Instantly boot your pristine, flawless native Helix panel overlay window
