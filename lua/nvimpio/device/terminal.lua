...
local Terminal = {
  ...
  _custom_stdout = nil,
  _is_scrolling = false,
  _last_matched_line = 0, -- Tracking field to prevent duplicate highlights on the same row
}
...
function Terminal:on_stdout(j, d, e)
  -- LIVE WORKSPACE HIGHLIGHT LOCK: Safely intercepts text blocks right as they mount on the layout grid
  if d and self.buf and vim.api.nvim_buf_is_valid(self.buf) and M.layout.container_win and vim.api.nvim_win_is_valid(M.layout.container_win) then
    vim.schedule(function()
      if not self.buf or not vim.api.nvim_buf_is_valid(self.buf) or not M.layout.container_win or not vim.api.nvim_win_is_valid(M.layout.container_win) then
        return
      end
      local total_lines = vim.api.nvim_buf_line_count(self.buf)

      -- Evaluate the last 3 streaming rows to capture your commands accurately
      ...
      for idx, text in ipairs(lines_content) do
        local target_line_idx = start_idx + idx

        -- RIGID STREAM FILTER: Only match rows starting with a true Windows drive prompt layout path
        if text:match('^[a-zA-Z]:\\.*>') and target_line_idx ~= self._last_matched_line then
          -- Apply a high-priority, window-level style overlay right on the generated coordinates row.
          -- This punches through any terminal tokenizer ANSI rules and keeps the theme block stable.
          self._last_matched_line = target_line_idx
          vim.api.nvim_win_call(M.layout.container_win, function()
            pcall(vim.fn.matchaddpos, 'Visual', { target_line_idx }, 100)
          end)
          break
        end
      end
