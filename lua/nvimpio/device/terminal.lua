...
function Terminal:on_open()
  ...
  -- Open the window container split natively under your current code window file column
  local anchor_win = (self.last_win and vim.api.nvim_win_is_valid(self.last_win)) and self.last_win or 0
  
  self.win = vim.api.nvim_open_win(self.buf, true, {
    split = "below",
    win = anchor_win, -- Focus local column nodes cleanly
  ...
  -- 🌟 THE IMMUTABLE GEOMETRY LOCK:
  -- winfixheight forces Neovim's internal window grid layout algorithm to freeze 
  -- this window's height. This stops user sidebars from crushing or shrinking your panel.
...
function Terminal:_register_lifecycle_events(target_height)
  ...
  -- 🌟 ANTISH-SHRINK ARCHITECTURE ENFORCER:
  -- Listens for any global layout changes (e.g., closing Neo-tree, opening Aerial).
  -- The millisecond any sidebar changes the screen, it runs a hard layout correction pass
  -- to lock the terminal height back to target settings, completely stopping collapses.
...
M.config = vim.tbl_deep_extend("force", M.config, opts or {})end
return M
