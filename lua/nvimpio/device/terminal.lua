...
function M.RestoreWorkspaceFocus()
  ...
  for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
    if vim.api.nvim_win_is_valid(win) then
      ...
      if ft ~= 'pio_terminal' and win_type == '' and ft ~= 'neo-tree' and ft ~= 'oil' and ft ~= 'aerial' and ft ~= 'pio_workspace' and not ft:match("^terminal_") then
        target_win = win
        break
      end
    end
...
function M.send_and_restore(cmd)
  ...
  if was_visible then
    ...
    vim.api.nvim_buf_call(target_instance.buf, function() vim.cmd('noautocmd stopinsert') end)
    return
  end
