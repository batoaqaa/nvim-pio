...
local Terminal = {
  ...
  _custom_stdout = nil,
  _is_scrolling = false,
  _ns_id = nil, -- Isolated formatting namespace layer
}
...
function Terminal.new(term_type, panel_title, filetype, custom_stdout)
  ...
  self._ns_id = vim.api.nvim_create_namespace('PioTerminalExtmarks_' .. term_type)
  return self
end
...
function Terminal:on_create()
  ...
  -- Create a standard bold weight highlight group
  vim.api.nvim_set_hl(0, 'PioBufComboBold', { bold = true, default = true })
...
--- Live Output Tracker Intercept Pipeline
function Terminal:on_stdout(j, d, e)
  -- NON-DESTRUCTIVE EXTMARK INJECTOR: Matches rows dynamically as they write to the grid
  if d and self.buf and vim.api.nvim_buf_is_valid(self.buf) then
    vim.schedule(function()
      ...
      -- Monitor the final 3 rows generated by the process pipe channel
      local start_idx = math.max(0, total_lines - 3)
      local lines_content = vim.api.nvim_buf_get_lines(self.buf, start_idx, total_lines, false)

      for idx, text in ipairs(lines_content) do
        local target_line_idx = start_idx + idx - 1

        -- Locate where the command payload begins immediately following your drive prompt arrow ('>')
        local match_start, match_end = text:find('^[a-zA-Z]:\\.*>s+')
        if not match_start then
          -- Loose path matching fallback guard
          match_start, match_end = text:find('^[a-zA-Z]:\\.*>')
        end

        if match_start and match_end then
          -- Inject a low-level buffer Extmark directly onto the command layout section coordinates.
          -- Mode 'combine' preserves your theme's colors while forcing heavy bold typeface weights.
          pcall(vim.api.nvim_buf_set_extmark, self.buf, self._ns_id, target_line_idx, match_end, {
            end_row = target_line_idx,
            end_col = #text,
            hl_group = 'PioBufComboBold',
            hl_mode = 'combine',
          })
        end
      end
...
function Terminal:on_close()
  ...
  if self.buf and vim.api.nvim_buf_is_valid(self.buf) then
    pcall(vim.api.nvim_buf_clear_namespace, self.buf, self._ns_id, 0, -1)
  end
  self.job = nil
  self.buf = nil
end
...
function M.send_and_restore(cmd)
  ...
  if was_visible then
    ...
    vim.api.nvim_buf_close(target_instance.buf)
    return
  end
