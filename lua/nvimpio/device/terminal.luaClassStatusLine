...
---@class TerminalStatusline Matrix for managing real-time build stream output widgets
---@field msg string Explicit text string showing the active operation name or percentage complete
---@field status "idle"|"building"|"success"|"failed" Structural enum tracking background thread compilation states
M.status = {
  msg = 'Idle',
  status = 'idle',
}
...
function Terminal:on_stdout(j, d, e)
  -- 1. ASYNC STATUSLINE STREAM FILTER: Parse raw terminal data matrices for active compilation states
  if self.term_type == 'cli' and M.status.status == 'building' then
    for _, raw_line in ipairs(d) do
      local line = raw_line:gsub('%s+', ' ') -- Strip complex terminal spacer variables

      -- Match explicit toolchain compilation milestones or errors
      if line:find('Building in') or line:find('Compiling') then
        local component = line:match('Compiling%s+([%w_%.%-]+)')
        M.status.msg = component and ('Compiling ' .. component) or 'Building...'
        vim.cmd('redrawstatus')
      elseif line:find('FAILED') or line:find('Error') then
        M.status.status = 'failed'
        M.status.msg = 'Build Failed'
        vim.cmd('redrawstatus')
      elseif line:find('SUCCESS') and not line:find('Building') then
        M.status.status = 'success'
        M.status.msg = 'Build Success'
        vim.cmd('redrawstatus')
      end
    end
  end

  -- 2. Execute local instance callbacks or route to the global capture pipeline hook
...
function Terminal:on_exit()
  ...
  -- Auto-reset compilation widgets to a safe fallback state on exit if text matching didn't resolve it
  if M.status.status == 'building' then
...
---@param title string Visual interface tab name assigned to the winbar rendering split
---@param filetype_or_cb string|fun(j: integer, d: string[], e: string)|nil Signature override or output callback hook
---@param custom_stdout fun(j: integer, d: string[], e: string)|nil Dedicated stream callback function
...
--- Public lookup function used by external statusline engines to fetch formatted indicators
---@return string Custom icon-mapped text string showing the active compilation state
function M.GetStatuslineWidget()
  local icons = { idle = '💤 ', building = '🔨 ', success = '✅ ', failed = '❌ ' }
  local prefix = icons[M.status.status] or ''
  return string.format('%s[PIO: %s]', prefix, M.status.msg)
end
