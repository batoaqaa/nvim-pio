...
function M.readContent(files)
  for _, tbl in pairs(files) do
    local cache_id = string.sub(vim.fn.sha256(vim.fs.dirname(tbl.file)), 1, 16)  --M.get_session_cache_id()
...
%s
  ...
  boiler = function(self, project_root_param)
    ...
    -- local cwdContent = readContent()
    -- local pkgContent = readContent()
    -- local frmContent = readContent()
    --------------------------------------------------------------------------------
