local paths = require('platformio.paths')
local ini = require('platformio.config_reader')
local val = require('platformio.validator')
local menu = require('platformio.menu')
local install = require('platformio.installer')
local main = require('platformio') -- Reference our parent init module safely
...
function M.ensure_toolchain_active(on_success_callback)
  ...
  local base_runtime = paths.clean(main.options.pio.pio_runtime_dir)
  ...
  if verified then
    ...
    if not current_path:find(escaped_bin, 1, true) then
      vim.env.PATH = main.config.pio_bin_dir .. paths.path_sep .. current_path
    end

    local final_storage = paths.clean(ini.check_ini_override() or main.options.pio.pio_storage_dir or base_runtime)
