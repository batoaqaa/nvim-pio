...
local function load_db(db_path)
  -- Automatically seed a default configuration template if the file is missing
  ...
  if ok and data then
    ...
    -- 🟢 CRITICAL PATTERN GUARD (ZERO HARDCODING):
    -- A valid C++ error code never contains uppercase characters, underscores, or spaces.
    -- This strips out garbage keys like 'args', 'count', or 'remove' right at the file gate!
    if type(data.codes) == 'table' then
      for k, v in pairs(data.codes) do
        if type(k) == 'string' and not k:match('[A-Z_]') and not k:match('%s') and not k:match('^%-') then
          clean_codes[k] = v
        end
      end
    end
    ...
    return { codes = clean_codes, flags = clean_flags }
  end
  return { codes = {}, flags = {} }
end
...
function M.manage_file_diagnostics_interactive(state_override)
  ...
  -- Reset the session tracker pool explicitly to prevent cross-contamination
  ...
  for _, d in ipairs(raw_diagnostics) do
    ...
    if c ~= '' and type(c) == 'string' then
      local is_valid_cpp_code = not c:match('[A-Z_]') and not c:match('%s') and not c:match('^%-')
      if is_valid_cpp_code then
  ...
  if #items == 0 then
    vim.notify('✅ Clean Slate: No active lints inside this buffer.', vim.log.levels.INFO)
    return
  end
